...
class DispersionCorrectionCalculator_mixedbasis(Calculator):
    """ Calculator to combine two calculators in simple additive scheme.

    This calculator hosts several calculators and simply adds energy, 
    force, stress contributions. Has special features for intercalc 
    communication for dispersion corrections.
    Reinhard Maurer, 2021
    """

    implemented_properties = ['energy', 'forces', 'stress']

    valid_args = ('qm_calculator',     # array of calculators
                  'mm_calculator',     # array of calculators
                  'hirbulk',            # float
                  'hirlast')            # array of tuples

    def __init__(self, restart=None, label=os.curdir, 
            atoms=None, reset=False, **kwargs):
        ...
        hirbulk:        float
            Default value for Hirshfeld volume ratio if mm_calc can take hirshfeld ratios for vdW calculations
        """
    ...
    def calculate(self, atoms, properties=['energy'], 
        ...
        if self.calculation_required(atoms, properties):    
            ...
            #MM part
            atomnumbers = mm_atoms.get_atomic_numbers()
            atompos = mm_atoms.get_positions()
            newatomnumbers=[]
            for i in atomnumbers:
                if i==46:
                    newatomnumbers.append(47)
                else:
                    newatomnumbers.append(i)
            constraint_indices=mm_atoms.constraints[0].get_indices()
            c=ase.constraints.FixAtoms(indices=constraint_indices)      
            mm_atoms_new = ase.atoms.Atoms(newatomnumbers,atompos)
             
            mm_atoms_new.set_constraint(c)
            del mm_atoms
            mm_atoms = mm_atoms_new 
